document.addEventListener('DOMContentLoaded', () => {
    ...
    function startGame() {
        ...
        // Only play start sound and animate if the start screen exists (Home Page)
        if (startScreen) {
    ...
    // Logic: If on Home Page (Start Screen exists), wait for input.
    // If on Blog Page (No Start Screen), load immediately.
    if (startScreen) {
        window.addEventListener('keydown', startGame, { once: true });
        startScreen.addEventListener('click', startGame, { once: true });
    } else {
        startGame(); // Auto-start on pages without splash screen
    }

    function typeText(element, text) {
        const textSpan = element.querySelector('.typing-text');
        if (!textSpan) return; // Guard clause
